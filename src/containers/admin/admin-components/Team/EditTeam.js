...
const editPage = ({itemToEdit, createNotification, onRouteChange, associationApiURL, teamApiURL, teamDeleteURL, associationDeleteURL, editName}) => {
  ...
  return(
    <section className='container'>
      ...
      <button onClick={e => {this.createNotification('error').then(axios.delete(teamDeleteURL, itemToEdit)).then(axios.delete(associationDeleteURL, itemToEdit)).then(response => onRouteChange()).catch(error => console.log(error))}} type="button" className="btn btn-danger btn-lg btn-block">Delete</button>
    </section>
  )
}
